PROVIDER_DIRS := $(filter-out bifrost-model-provider,$(sort $(patsubst %/go.mod,%,$(wildcard *-provider/go.mod))))

IMAGE_REPOSITORY ?= ghcr.io/obot-platform/enterprise-providers
IMAGE_TAG ?= main
IMAGE_PLATFORMS ?=
IMAGE_PUSH ?= false
IMAGE_BUILD_COMMAND ?=

.PHONY: all-images provider-dirs
